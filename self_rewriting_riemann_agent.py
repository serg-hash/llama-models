...
"""Self-rewriting agent that tries to model Riemann zeta zeros."""
...
def main() -> None:
    cfg = load_config()
    log(f"Running version {cfg['version']}")
    error = train(cfg)
    ...
    if error >= cfg.get("best_error", 1e9):
        ...
        cfg["version"] += 1
        log(
            "Improvement insufficient. Increasing hidden_dim to"
            f" {cfg['hidden_dim']} and lowering lr to {cfg['learning_rate']:.4f}."
        )
        save_config(cfg)
        rewrite_self(cfg)
    else:
        ...
        save_config(cfg)
        log("Configuration updated with improved error")
