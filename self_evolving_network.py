...
class SelfEvolvingNet:
    ...
    def introduce(self) -> str:
        """Return a human-readable identifier for the entity."""
        return (
            f"I am {self.state.name}, version {self.state.version}, "
            f"architecture {self.state.architecture}"
        )
    ...
    def _evolve(self, loss: float) -> None:
        ...
        self._build_model()
        print(
            f"[{self.state.name}] version {self.state.version}: {reason}; "
